...
public abstract class MsgPackProcessor<S> implements StructProcessor<S> {
    ...
    private void processStart(MessageUnpacker unpacker, StructHandler handler) throws IOException {
        if (unpacker.hasNext()) {
            processStruct(unpacker, handler, getExtensionTypeHeader(EventFlags.startStruct, unpacker));
        }
    }
    ...
    private StructHandleResult processStruct(MessageUnpacker unpacker, StructHandler handler, ExtensionTypeHeader typeHeader) throws IOException {
        ...
        return skipOrGo(handler.getLastHandleResult(),
                ...
                () -> {
                    StructHandleResult entryRes = StructHandleResult.CONTINUE;
                    for (int i = 0; i < length; ++i) {
                        entryRes = skipOrGo(
                                entryRes, () -> {
                                    skipName(unpacker);
                                ...
                                () -> skipOrGo(
                                        ...
                                        () -> {
                                            ...
                                            return (valRest == StructHandleResult.SKIP_SIBLINGS | valRest == StructHandleResult.SKIP_SUBTREE) ? StructHandleResult.CONTINUE : valRest;
                                        }
                                )
                        );
                    }
            ...
            case EXTENSION:
                ...
                switch (typeHeader.getType()) {
                    ...
                    case EventFlags.startSet:
                        ...
                        break;
                    default:
                        getExtensionTypeHeader(EventFlags.startSet, typeHeader);//err
                }
                break;
            default:
        ...
        return skipOrGo(handler.getLastHandleResult(),
                ...
                () -> {
                    ...
                    for (int i = 0; i < length; ++i) {
                        ...
                        entryRes = entryRes == StructHandleResult.SKIP_SUBTREE ? StructHandleResult.CONTINUE : entryRes;
                    }
    ...
    private StructHandleResult processMapKey(MessageUnpacker unpacker, StructHandler handler) throws IOException {
        ...
        if (handleResult == StructHandleResult.CONTINUE) {
            handler.endKey();
        }
        //handler.getLastHandleResult();// > /dev/null
        return handleResult == StructHandleResult.JUMP_VALUE ? StructHandleResult.CONTINUE : handleResult;
    }
    ...
    private StructHandleResult processMapValue(MessageUnpacker unpacker, StructHandler handler) throws IOException {
        ...
        //handler.getLastHandleResult();// > /dev/null
        return handleResult == StructHandleResult.SKIP_SUBTREE ? StructHandleResult.CONTINUE : handleResult;
    }
    ...
    private void goIfAlive(StructHandleResult result, HandleAction goAction) throws IOException {
        if (result != StructHandleResult.TERMINATE && result != StructHandleResult.JUMP_VALUE) {
            goAction.consume();
        }
    }
    ...
    private ExtensionTypeHeader getExtensionTypeHeader(int expectedType, ExtensionTypeHeader typeHeader) throws BadFormatException {
        ...
        throwBadFormat("wrong extension type", EventFlags.startStruct, typeHeader);
        return null;
    }
